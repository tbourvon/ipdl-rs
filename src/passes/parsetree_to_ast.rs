...
pub struct ParseTreeToAST<'includedirs> {
    include_resolver: &'includedirs IncludeResolver<'includedirs>,
}

impl<'includedirs> ParseTreeToAST<'includedirs> {
    pub fn new(include_resolver: &'includedirs IncludeResolver<'includedirs>) -> Self {
        ParseTreeToAST {
            include_resolver
        }
    }

    pub fn parsetree_to_ast(&self, parse_tree: parser::ParseTree) -> Result<ast::AST, Errors> {
        Ok(ast::AST {
            translation_unit: self.convert_translation_unit(parse_tree.translation_unit.data, parse_tree.file_path)?
        })
    }

    fn convert_translation_unit(&self, pt_translation_unit: pipdl::TranslationUnit, pt_filename: PathBuf) -> Result<ast::TranslationUnit, Errors> {
        ...
        for item in pt_translation_unit.items {
            match item {
                ...
                pipdl::Item::Protocol(protocol_item) => {
                    match protocol {
                        Some(_) => return Err(Errors::one(&ast::Location { file_name: pt_filename, lineno: protocol_item.span.start.line, colno: protocol_item.span.start.col }, "only one protocol definition per file")),
        ...
        Ok(ast::TranslationUnit {
            cxx_includes: pt_translation_unit.cxx_includes.into_iter().map(|x| x.data.file.data).collect(),

            includes: pt_translation_unit.includes.into_iter().map(|x| {
                let filename = format!("{}{}{}", x.data.id.data, ".ipdl", if x.data.protocol.is_some() {""} else {"h"});
                self.include_resolver.get_include(&filename).expect("Cannot find include TUId when converting ParseTree to AST")
            }).collect(),

            using: pt_translation_unit.usings.into_iter().map(|x| self.convert_using_stmt(x.data)).collect(),
            ...
            namespace: match &protocol {
                ...
                None => {
                    ...
                    if last_is_struct {
                        ...
                        match unions.last() {
                            Some(u) => u.0.clone(),
                            None => return Err(Errors::one(&ast::Location { file_name: pt_filename, lineno: 0, colno: 0 }, "file is empty")),
                        }
                    }
                }
            },
            ...
            unions,
            protocol,
            file_type: ast::FileType::from_file_path(pt_filename.as_path()).expect("Cannot determine file type when converting parse tree to AST"),
    ...
    fn convert_cxx_path(&self, mut pt_cxx_path: pipdl::CxxPath) -> ast::QualifiedId {
        ast::QualifiedId {
            base_id: self.convert_cxx_path_seg(pt_cxx_path.segs.pop().expect("Empty path when converting into QualifiedId").data),
            quals: pt_cxx_path.segs.into_iter().map(|x| self.convert_cxx_path_seg(x.data).id).collect(),
        }
    }
    ...
    fn convert_cxx_path_seg(&self, pt_cxx_path_seg: pipdl::CxxPathSeg) -> ast::Identifier {
        ast::Identifier {
            id: match pt_cxx_path_seg.args {
                Some(args) => format!("{}<{}>", pt_cxx_path_seg.id.data, args.data.into_iter().map(|x| x.data).collect::<Vec<String>>().concat()),
    ...
    fn convert_struct_item(&self, pt_struct_item: pipdl::StructItem) -> (ast::Namespace, Vec<ast::StructField>) {
        (
            self.convert_path(pt_struct_item.path),
            pt_struct_item.fields.into_iter().map(|x| self.convert_struct_field(x.data)).collect()
        )
    }

    fn convert_path(&self, mut pt_path: Vec<pipdl::Spanned<String>>) -> ast::Namespace {
        ast::Namespace {
            name: self.convert_name(pt_path.pop().expect("Empty path when converting into Namespace")),
    ...
    fn convert_union_item(&self, pt_union_item: pipdl::UnionItem) -> (ast::Namespace, Vec<ast::TypeSpec>) {
        (
            self.convert_path(pt_union_item.path),
            pt_union_item.components.into_iter().map(|x| self.convert_type(x.data)).collect()
        )
    }

    fn convert_protocol_item(&self, pt_protocol_item: pipdl::ProtocolItem) -> (ast::Namespace, ast::Protocol) {
        (
            ...
            ast::Protocol {
                ...
                managers: match pt_protocol_item.managers {
                    Some(managers) => managers.data.into_iter().map(|x| self.convert_name(x)).collect(),
                    None => Vec::new(),
                },
                manages: pt_protocol_item.manages.into_iter().map(|x| self.convert_name(x.data)).collect(),
                messages: pt_protocol_item.groups
                    .into_iter().flat_map(|group| {
                        let (decls, direction) = (group.data.decls, group.data.direction.data);
                        decls
                            .into_iter().map(move |x| (x.data, direction.clone()))
                                        .map(|(message_decl, direction)| self.convert_message_decl(message_decl, &direction))
                    }).collect()
            }
        )
    }

    fn convert_message_decl(&self, pt_message_decl: pipdl::MessageDecl, pt_direction: &pipdl::Direction) -> ast::MessageDecl {
        ...
        ast::MessageDecl {
            ...
            in_params: pt_message_decl.params.into_iter().map(|x| self.convert_param(x.data)).collect(),
            out_params: match pt_message_decl.returns {
                Some(returns) => returns.data.into_iter().map(|x| self.convert_param(x.data)).collect(),
                None => Vec::new(),
            },
            compress,
            verify
    ...
    fn convert_send_semantics(&self, pt_send_semantics: &pipdl::SendSemantics) -> ast::SendSemantics {
    ...
    fn convert_nesting(&self, pt_nesting: Option<pipdl::Spanned<pipdl::Spanned<pipdl::Nesting>>>) -> ast::Nesting {
        match pt_nesting {
    ...
    fn convert_priority(&self, pt_priority: Option<pipdl::Spanned<pipdl::Spanned<pipdl::Priority>>>) -> ast::Priority {
        match pt_priority {
            Some(x) => match x.data.data {
